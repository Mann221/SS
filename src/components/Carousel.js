...
const Carousel = () => {
  ...
  const dragging = (e) => {
    ...
    if (newScrollLeft <= 0 || newScrollLeft >= carouselRef.current.scrollWidth - carouselRef.current.offsetWidth) {
      setIsDragging(false);
      return;
    }
  ...
  return (
    <div className="carousel-container relative w-full overflow-hidden">
      <div
        className="carousel flex overflow-x-auto scroll-smooth no-scrollbar"
        ref={carouselRef}
      ...
      <div className="carousel-nav absolute bottom-4 left-0 right-0 flex items-center justify-between px-4">
        ...
        <button className="carousel-next text-black p-2" onClick={scrollToNextSlide}>
